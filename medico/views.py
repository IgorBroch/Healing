...
def cadastro_medico(request):
    ...
    missing_fields = []
    for field in ['crm', 'nome', 'cep', 'rua', 'bairro', 'numero', 'rg', 'cim', 'foto', 'especialidade', 'descricao', 'valor_consulta']:
      if not request.POST.get(field):
        missing_fields.append(field)

    context = {'especialidades': especialidades, "is_medico": is_medico(request.user), 'errors': missing_fields}
    if missing_fields:
     
      message = "Preencha todos os campos obrigatórios para continuar: " + ", ".join(missing_fields)
...
def abrir_horario(request):
    ...
    if request.method == "GET":
        dados_medicos = DadosMedico.objects.filter(user=request.user).first()
...
def consultas_medico(request):
    ...
    return render(request, 'consultas_medicos.html', {'consultas_hoje': consultas_hoje, 'consultas_restantes': consultas_restantes, 'is_medico': is_medico(request.user)})
